...
export default function AdminOrdersPage() {
  ...
  const [showAutoCancelled, setShowAutoCancelled] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    setError("");
    try {
      const path = showAutoCancelled
        ? "/api/admin/orders?includeExpired=true"
        : "/api/admin/orders";
      const res = await adminFetch(path, getToken() || "");
      ...
      list.map((o) =>
        o.id === order.id
          ? {
              ...
              autoCancelledAt: status === "cancelled" ? o.autoCancelledAt : null,
            }
          : o,
      ),
    );
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      ...
      <label className="mt-4 flex w-fit items-center gap-2 text-sm text-ink/60">
        <input
          type="checkbox"
          checked={showAutoCancelled}
          onChange={(e) => setShowAutoCancelled(e.target.checked)}
          className="h-4 w-4 rounded border-ink/20 text-brand-500"
        />
        Show auto-cancelled after 24h
      </label>
              ...
              >
                ...
                {order.autoCancelledAt && (
                  <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800">
                    Auto-cancelled after 24h on {formatDateTime(order.autoCancelledAt)}. Kept here for audit only.
                  </p>
                )}
